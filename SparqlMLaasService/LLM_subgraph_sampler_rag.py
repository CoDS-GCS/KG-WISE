...
def match_features_prompt(schema,suggested_features,KG="DBLP",model_name=model_name,topk=25):
  ...
  {suggested_features}
  ---------------------------------------------
1- Prioritize higher frequency nodes.
2- Identify distinct predicates/relation in the schema (subject, predicate, object).
3- If a predicate appears multiple times with different subject or object types, generalize by replacing subject or object with str. Avoid repetition.
4- Select diverse predicates.
5- Return the top {topk} matched schema triples.
6- NO EXPLANATIONS!
  """
...
def generate_sparql_query(BGP_lst,KG="DBLP",VT="Publication",
  ...
  - Given the following triples list from the {KG} knowledge graph schema , Write SPARQL query to selects the {VT} and its associated information given int the following triples list.
  - the triples are directed, make sure to fulfill the direction and relation type.
  - the query must return the union of sub select statement in the form ?s ?p ?o.
  ...
  3. in tow-hop or more nested select:
      ...
      3.2 used the last connected entity as subject as shown in the previous example
  4. Generate only the SPARQL query without any explanation.
  5. make sure to use each given BGP triple
  6. add the BGP:  'Values ?s {{<VT-List>}}.' to the end of each subquery as it is. DO NOT replace 'VT-List' with anything else.
  7. Refine all rules  and the query syntax.
  8. Do not invent new relations i.e, dblp:authoredBy can not be dblp:Authored. However, you can define new PREFIXES
  9. If an IRI contains a / after the domain (e.g., https://domain.org/path1/path2/resource), then:

    9.1. Always create a new PREFIX that ends immediately before the last / in the IRI.
    (e.g., define PREFIX path2: <https://domain.org/path1/path2/>)
    
    9.2. Use only the final segment (resource) as the local name:
    path2:resource
    
    9.3. Do not reuse an existing PREFIX if it would cause the local name to include /.
    For example, prefix:path2/resource is invalid and must be avoided.
    
    9.4 If existing PREFIX with root exists, you can create new one from the root.
        Example:
        if "PREFIX example: <https://example.com/>" exists then you can also create
           "PREFIX section: <https://example.com/section/>" or
           "PREFIX subjects: <https://example.com/subjects/>"

    9.5. The local name (the part after : in prefix:localName) must never include a slash /.
        Example:
        ✅ Correct:
        PREFIX section: <https://example.com/section/>
        ?s ?p section:item
    
        ❌ Invalid:
        PREFIX example: <https://example.com/>
        ?s ?p example:section/item   # ❌ This is WRONG because 'section/item' is invalid in SPARQL

  10. You can start with ?o instead of ?s.
    Example:
      ...
      ?o a dblp:author.
      ?s dblp:authoredBy ?o.
  11. Do not duplicate BGPs
  12. No extra tokens, SPARQL Query Only!
  13. For every BGP in a subquery, you MUST include a BIND() statement that assigns the predicate to the variable ?p.
  """
...
def  refine_sparql_query(sparql_query,model_name=model_name):
  sparql_refine_prompt=f"""-You are an expert SPARQL query writer.
                  ...
                  - Rule 1: Keep the nested selects and their Union statements
                  - Rule 2: Avoid repetitions
                  - Rule 3: restructure the n-hop sub-select to choose final BGP 'subject' and 'object' and as the select items
                    ...
                    Example:  {{
                      ...
                      the final BGP is '?y prefix:z ?z' , then the select items must be:
                      1- ?y as ?s
                      2- ?p
                      3- ?z as ?o
...
def LLM_KG_Sampler_examples(example):
    if example == "DBLP_NC":
        ...
        KG = "DBLP"
        ####################### PV NC################################
        dblp_pv_suggested_features, dblp_pv_suggested_features_usage, dblp_pv_suggested_features_full_response, dblp_pv_suggested_features_p = suggest_features_prompt(
            task="classify a publication into a certian publication venue on DBLP KG.")
        dblp_pv_matching_BGP_lst, dblp_pv_matching_p_usage, dblp_pv_matching_p_full_response, dblp_pv_matching_p = match_features_prompt(
            dblp_filtred_schema, dblp_pv_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            dblp_pv_matching_BGP_lst, KG="DBLP", VT="Publication", SPARQL_Example=dblp_SPARQL_example)
        ...
        dblp_AA_matching_BGP_lst, dblp_AA_matching_p_usage, dblp_AA_matching_p_full_response, dblp_AA_matching_p = match_features_prompt(
            DBLP2022_KG_str_Schema, dblp_AA_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            dblp_AA_matching_BGP_lst, VT=VT, KG="DBLP", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(sparql_v0)
        print("final_sparql=", final_sparql)
        ...
        yago4_schema_df
        YAGO4_KG_Place_Schema_df = yago4_schema_df[
            (yago4_schema_df["stype"] == "schema:Place") | (yago4_schema_df["otype"] == "schema:Place")]
        YAGO4_KG_Place_Schema_txt = YAGO4_KG_Place_Schema_df.to_csv(index=False, header=None)
        ################### YAGO4 PC NC ###########################
        yag4_pc_suggested_features, yag4_pc_suggested_features_usage, yag4_pc_suggested_features_full_response, yag4_pc_suggested_features_p = suggest_features_prompt(
            task="classify a place into crossponding country from YAGO4 KG.")
        KG = "YAGO4"
        print("yag4_pc_suggested_features=", yag4_pc_suggested_features)
        yag4_pc_matching_BGP_lst, yag4_pc_matching_p_usage, yag4_pc_matching_p_full_response, yag4_pc_matching_p = match_features_prompt(
            YAGO4_KG_Place_Schema_txt, yag4_pc_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            yag4_pc_matching_BGP_lst, VT=VT, KG="YAGO4", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(
            sparql_v0)
        print("final_sparql=", final_sparql)
        ######################## YAGO4 CW Genre NC #################################
        YAGO4_KG_CW_Schema_df = yago4_schema_df[(yago4_schema_df["stype"] == "schema:CreativeWork") | (
                    yago4_schema_df["otype"] == "schema:CreativeWork")].drop_duplicates()
        YAGO4_KG_CW_Schema_txt = YAGO4_KG_Place_Schema_df.to_csv(index=False, header=None)
        YAGO4_KG_CW_Schema_df
        yag4_CWG_suggested_features, yag4_CWG_suggested_features_usage, yag4_CWG_suggested_features_full_response, yag4_CWG_suggested_features_p = suggest_features_prompt(
            task="classify a CreativeWork into crossponding Genre from YAGO4 KG.")
        KG = "YAGO4"
        print("yag4_CWG_suggested_features=", yag4_CWG_suggested_features)
        yag4_CWG_matching_BGP_lst, yag4_CWG_matching_p_usage, yag4_CWG_matching_p_full_response, yag4_CWG_matching_p = match_features_prompt(
            YAGO4_KG_CW_Schema_df, yag4_CWG_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            yag4_CWG_matching_BGP_lst, VT=VT, KG="YAGO4", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(
            sparql_v0)
        print("final_sparql=", final_sparql)
        ...
        YAGO4_KG_CA_Schema_df_list = set(YAGO4_KG_CA_Schema_df_l1["stype"].tolist()).union(
            set(YAGO4_KG_CA_Schema_df_l1["otype"].tolist()))
        ...
        YAGO4_KG_CA_Schema_df = pd.concat(
            [YAGO4_KG_CA_Schema_df_l1, YAGO4_KG_CA_Schema_df_l2_s, YAGO4_KG_CA_Schema_df_l2_o])
        ...
        mag_schema_df = pd.DataFrame(data=[x.split(',') for x in mag_schema.split('\n')],
                                     columns=["stype", "ptype", "otype"])
        MAG_KG_Place_Schema_df = mag_schema_df[
            (mag_schema_df["stype"] == "Paper") | (mag_schema_df["otype"] == "Paper")]
        MAG_KG_Place_Schema_txt = MAG_KG_Place_Schema_df.to_csv(index=False, header=None)
        ################## MAG PV NC ##################
        MAG_pv_suggested_features, MAG_pv_suggested_features_usage, MAG_pv_suggested_features_full_response, MAG_pv_suggested_features_p = suggest_features_prompt(
            task="classify a paper into crossponding publication venue from MAG KG.")
        KG = "MAG"
        print("MAG_pv_suggested_features=", MAG_pv_suggested_features)
        MAG_pv_matching_BGP_lst, MAG_pv_matching_p_usage, MAG_pv_matching_p_full_response, MAG_pv_matching_p = match_features_prompt(
            MAG_KG_Place_Schema_txt, MAG_pv_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            MAG_pv_matching_BGP_lst, VT=VT, KG="MAG", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(
            sparql_v0)
        print("final_sparql=", final_sparql)
        ...
        yago3_10_KG_Place_Schema_df = yago3_10_schema_df[
            (yago3_10_schema_df["stype"] == "Airport") | (yago3_10_schema_df["otype"] == "Airport")]
        yago3_10_KG_Place_Schema_txt = yago3_10_KG_Place_Schema_df.to_csv(index=False, header=None)
        ####################### YAGO3-10 CA LP ########################
        yago3_10_suggested_features, yago3_10_suggested_features_usage, yago3_10_suggested_features_full_response, yago3_10_suggested_features_p = suggest_features_prompt(
            task="predict a connection link between two Airports as link prediction task from yago3_10 KG.")
        KG = "yago3_10"
        print("yago3_10_suggested_features=", yago3_10_suggested_features)
        yago3_10_matching_BGP_lst, yago3_10_matching_p_usage, yago3_10_matching_p_full_response, yago3_10_matching_p = match_features_prompt(
            yago3_10_KG_Place_Schema_txt, yago3_10_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            yago3_10_matching_BGP_lst, VT=VT, KG="yago3_10", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(
            sparql_v0)
        print("final_sparql=", final_sparql)
        ...
        yago3_10_KG_Place_Schema_txt = yago3_10_KG_Place_Schema_df.to_csv(index=False, header=None)
        yago3_10_suggested_features, yago3_10_suggested_features_usage, yago3_10_suggested_features_full_response, yago3_10_suggested_features_p = suggest_features_prompt(
            task="predict where a person lives in as a link prediction task from yago3_10 KG.")
        KG = "yago3_10"
        print("yago3_10_suggested_features=", yago3_10_suggested_features)
        yago3_10_matching_BGP_lst, yago3_10_matching_p_usage, yago3_10_matching_p_full_response, yago3_10_matching_p = match_features_prompt(
            yago3_10_KG_Place_Schema_txt, yago3_10_suggested_features, KG)
        ...
        WikiDataPropertyLabels_dict = dict(
            zip(WikiDataPropertyLabels_df["PID"].astype(str).tolist(), WikiDataPropertyLabels_df["PName"].tolist()))
        wikiKG_KG_Schema_df["PName"] = wikiKG_KG_Schema_df["ptype"].apply(
            lambda x: WikiDataPropertyLabels_dict[str(x)[1:]] if str(x)[
                                                                 1:] in WikiDataPropertyLabels_dict.keys() else "")
        wikiKG_KG_Schema_df
        wiki_KG_Person_Schema_df = wikiKG_KG_Schema_df[
            (wikiKG_KG_Schema_df["stype"] == "person") | (wikiKG_KG_Schema_df["otype"] == "person")
            | (wikiKG_KG_Schema_df["stype"] == "occupation") | (wikiKG_KG_Schema_df["otype"] == "occupation")
            ]
        wiki_KG_Person_Schema_txt = wiki_KG_Person_Schema_df[["stype", "ptype", "otype"]].drop_duplicates().to_csv(
            index=False, header=None)
        wiki_KG_Person_Schema_df.drop_duplicates()
        ##################### WikiKG PO LP #########################
        wikiKG_suggested_features, wikiKG_suggested_features_usage, wikiKG_suggested_features_full_response, wikiKG_suggested_features_p = suggest_features_prompt(
            task="predict where a person occupaation (profession) as a link prediction task from  wikiKG knoweldge graph.")
        KG = " wikiKG"
        print(" wikiKG_suggested_features=", wikiKG_suggested_features)
        wikiKG_matching_BGP_lst, wikiKG_matching_p_usage, wikiKG_matching_p_full_response, wikiKG_matching_p = match_features_prompt(
            wiki_KG_Person_Schema_txt, wikiKG_suggested_features, KG)
        ...
        sparql_v0, sparql_v0_p_usage, sparql_v0_p_full_response, sparql_v0_p = generate_sparql_query(
            wikiKG_matching_BGP_lst, VT=VT, KG=" wikiKG", SPARQL_Example=dblp_SPARQL_example)
        print("sparql_v0=", sparql_v0)
        final_sparql, final_sparql_p_usage, final_sparql_p_full_response, final_sparql_p = refine_sparql_query(
            sparql_v0)
        print("final_sparql=", final_sparql)
        ...
        raise NotImplementedError(f"Unsupported example: {example}")
    return final_sparql

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LLM Sampler for KG Extraction')
    parser.add_argument('--example',type='str',default='DBLP_NC',choices=['DBLP_NC','MAG_NC','YAGO4','WikiKG','YAGO310'])
    args = parser.parse_args()
    print(LLM_KG_Sampler_examples(args.example))
