...
# sys.path.insert(0, '/media/hussein/UbuntuData/GithubRepos/KGNET/GNNaaS/models/')
...
# from GNNaaS.DataTransform.TSV_TO_PYG_dataset import inference_transform_tsv_to_PYG
...
class RGCN(torch.nn.Module):
    ...
    def store_emb(self, model_name, root_path=os.path.join(KGNET_Config.trained_model_path, 'emb_store')):
        ...
        for key, val in self.emb_dict.items():
            ...
            emb_array = root.create(key, shape=val_np.shape, dtype=val_np.dtype,
                                    chunks=(128, -1))  # chunks=(val_np.shape[0], -1)
    ...
    def Zarr_inference(self, x_dict, edge_index_dict, key2int):
        ...
        def denseToSparse(x, output, out_dims):
            indices = x.coalesce().indices()[0].unique()
            result_rows = output[indices].view(-1)
            v = create_indices(indices, out_dims[1])
            return torch.sparse.FloatTensor(v, result_rows, torch.Size(out_dims)).to(
                torch.float32).to_sparse_coo()  # .to_sparse_csr()

        def dynamicSparse(x, threshold=0.9):
            # if not x.is_sparse:
            #     return x
            x = x.to_sparse_coo()
            nnz = x._nnz() // x.size(1)
        ...
        for i, conv in enumerate(self.convs):
            ...
            for j, x in x_dict.items():
                # out_dict[j] = conv.root_lins[j](x) #.to_sparse_csr()
                if x.is_sparse and j not in target_type:  # and j not in denseRel:
                    ''' For sparse conversion without sparsity check'''
                    # output = conv.root_lins[j](x.to_dense())
                    # out_dict[j] = denseToSparse(x,output,(x.shape[0], output.shape[1]))#.to_dense()
                    # continue
                    ...
                    print(f'sparsity = {sparsity}\n')
                    # x = dynamicSparse(x)
                    if sparsity >= 0.9:  # x.is_sparse:##
                        print(f'converting {j} to sparse\n', '*' * 10)
                        output = conv.root_lins[j](x)
                        out_dict[j] = denseToSparse(x, output, (x.shape[0], output.shape[1]))  # .to_dense()
                        continue
                    else:
                        denseRel.append(j)
                        print(f'{j} appended to blacklist')

                print(f'keeping {j} dense\n', '*' * 10)
                out_dict[j] = conv.root_lins[j](x)  # .to_sparse_csr()
            ...
            try:
                ...
                for keys, adj_t in adj_t_dict.items():
                    ...
                    out = out_dict[key2int[target_key]]  # .to_sparse()
                    ...
                    # if x_dict[key2int[src_key]].is_sparse:
                    tmp = torch.sparse.mm(adj_t, x_dict[key2int[src_key]])  # .to_sparse())
                    # tmp = tmp.to_sparse_coo()
                    # else:
                    #     tmp = adj_t.matmul(x_dict[key2int[src_key]].to_sparse(), )
                    ...
                    # print(f'tmp shape before conv {tmp.shape}')
                    # tmp = conv.rel_lins[key2int[keys]](tmp).resize_([out.size()[0], out.size()[1]])
                    output = conv.rel_lins[key2int[keys]](tmp)
                    if tmp.is_sparse:
                        indices = tmp.coalesce().indices()[0].unique()
                        result_rows = output[indices].view(-1)
                        v = create_indices(indices, output.shape[1])
                        tmp = denseToSparse(tmp, output, (out.shape[0], out.shape[1]))
                    # tmp = torch.sparse.FloatTensor(v, result_rows, torch.Size((out.size()[0], out.size()[1]))).to(torch.float32)
                    else:
                        tmp = output.resize_([out.size()[0], out.size()[1]])

                    tim_relEnd = (datetime.datetime.now() - tim_relStart).total_seconds()
                    try:
                        out_dict[key2int[target_key]] = out + tmp  # tmp+out
                    except:
                        out_dict[key2int[target_key]] = tmp + out
                    ...
                    dict_relTime.setdefault(keys, []).append(
                        (flag, time_tmpGen, tim_relEnd, tim_addEnd, '=', time_infLoop1_0End))

                time_infLoop1End = (datetime.datetime.now() - time_infLoop1Start).total_seconds()
                # print(f'\n\n {i} Average Root Lins Loop time = ', sum(loop_rootlins) / len(loop_rootlins))
            ...
            if i != self.num_layers - 1:
                for j in range(self.num_node_types):
                    if j in out_dict:
                        # F.relu_(out_dict[j])
                        if out_dict[j].is_sparse:
                            # print('Sparse in Linear')
                            F.relu_(out_dict[j].coalesce())
                            # print(f'and now it is {out_dict[j].is_sparse}')
                            # out_dict[j] = out_dict[j].to_sparse_coo()
                        else:
                            # print('Dense in Linear')
                            F.relu_(out_dict[j])
                            out_dict[j] = out_dict[j].to_sparse()
                            # if j not in denseRel:
                            #     out_dict[j] = out_dict[j].to_sparse_coo()
                            # out_dict[j] = dynamicSparse(out_dict[j])
            time_infLoop2End = (datetime.datetime.now() - time_infLoop2Start).total_seconds()
            # print(f'{i} Time loop 2 = {time_infLoop2End}')

            # for j, x in out_dict.items():
            # out_dict[j] = x.to_sparse()
            # print(f'out_dict {j} == {out_dict[j]}')
            x_dict = out_dict
        # for k,v in dict_relTime.items():
        #     print(f'{k} ------{v}')
        #     print('')
        # pd.DataFrame(dict_relTime).T.to_csv('/shared_mnt/KGNET/logs/WISE_EXP.csv')

        return x_dict

    def RW_sampling_inference(self, model, homo_data, x_dict, local2global, subject_node, node_type, target_mask,
                              device='cpu'):
        model.eval()
        ...
        batch_size = 2  # len(target_mask) // 16 #* 2   # 1024 * 2
        ...
        print('Inference Loader Initialized!')
        """ target node """
        ...
        for data in inference_loader:
            # data = data.to(device)
            # print(f'data.edge_index {len(data.edge_index[0])}\ndata.edge_attr {len(data.edge_attr)}\ndata.node_type {len(data.node_type)}\ndata.local_node_idx {len(data.local_node_idx)}')
            out = model(x_dict,
                        data.edge_index,
                        data.edge_attr,
                        data.node_type,
    ...
    def load_Zarr_emb(self, edge_index_dict, key2int, model_name, target, root_path=KGNET_Config.emb_store_path,
                      **kwargs):
        def create_indices(v, emb_size):
            repeated_v = torch.tensor(v).repeat_interleave(emb_size)  # Repeat each element in v emb_size times
            repeated_range = torch.arange(emb_size).repeat(
        ...
        def calculate_chunks_accessed(chunk_size, indices):
            """
            Calculate how many chunks will be accessed for the given indices.

            Parameters:
            chunk_size (tuple): A tuple (chunk_rows, chunk_cols) representing the chunk size.
            indices (list): A list of row indices to retrieve.

            Returns:
            int: The number of unique chunks that will be accessed.
            """
            chunk_rows, chunk_cols = chunk_size
            chunk_indices = set()  # A set to store unique chunk indices accessed

            for index in indices:
                # Calculate the chunk index for the current row index
                chunk_index = index // chunk_rows
        ...
        """ For chunk information """
        # n_types = len(list(root.keys()))
        # dict_chunks = {}
        # rev_key2int = {v:k for k,v in list(key2int.items())[:n_types]}
        """ #####################"""
        time_load_start = datetime.datetime.now()
        for k, v in to_load.items():
            v = list(v)
            total_chunks = len(root[k])//root[k].chunks[0]
            # dict_chunks[rev_key2int[k]] = (calculate_chunks_accessed(root[k].chunks, v), total_chunks if total_chunks != 0 else 1) ### Storing chunk information for statistics
...
def wise_SHsaint(device=0, num_layers=2, hidden_channels=64, dropout=0.5,
                 ...
                 num_steps=10, loadTrainedModel=0, dataset_name="",
                 root_path=KGNET_Config.datasets_output_path, output_path="./", include_reverse_edge=True,
    ...
    for GNN_dataset_name in GNN_datasets:
        ...
        if loadTrainedModel == 1:
            ...
            del output, _
            label_mapping = getLabelMapping(GNN_dataset_name)
            # master_dir = os.path.join(KGNET_Config.datasets_output_path,GNN_dataset_name)
        ...
        if loadTrainedModel == 1 and target_masks is not None:
            # feat = torch.sparse.FloatTensor(size=(data.num_nodes_dict[subject_node],emb_size))#.to_sparse_coo()
            ...
            with torch.no_grad():
                ...
                if not target_mapping:
                    target_mapping = pd.read_csv(
                        os.path.join(dir_path, 'mapping', f'{subject_node}_entidx2name.csv.gz'), compression='gzip')
                ...
                timeStart_modelLoad = datetime.datetime.now()
                ...
                if RW_SAMPLER:
                    ...
                    timeEnd_modelLoad = (datetime.datetime.now() - timeStart_modelLoad).total_seconds()
                    timeStart_inf = datetime.datetime.now()
                    y_pred = model.RW_sampling_inference(model, homo_data, x_dict, local2global, subject_node,
                                                         node_type,
                                                         target_mask=target_masks)
                    y_pred = y_pred.unsqueeze(1)

                    end_t = datetime.datetime.now()
                    timeEnd_inf = (datetime.datetime.now() - timeStart_inf).total_seconds()

                    dic_results["InferenceTime"] = timeEnd_inf  # (end_t - start_t).total_seconds()
                    print(f'y_true: {y_true.size()}\ny_pred: {y_pred.size()}\nTarget masks: {len(target_masks)}')

                    test_acc, num_missing_labels = calculate_acc(y_true, y_pred, target_masks, target_masks_inf,
                                                                 evaluator)
                    ...
                    total_time = process_end + timeEnd_modelLoad + dic_results["InferenceTime"]
                    ...
                    print('*' * 8, '\tTOTAL MODEL LOAD:\t', timeEnd_modelLoad, 's')
                ...
                print('*' * 8, '\tTOTAL MODEL LOAD:\t', timeEnd_modelLoad, 's')
                print('*' * 8, '\tInference TIME:\t\t', dic_results["InferenceTime"], 's')
                print('*' * 8, '\tTotal TIME:\t\t', process_end + timeEnd_modelLoad + dic_results["InferenceTime"],
                      's', )
                ...
                """ UNCOMMENT THE FOLLOWING TO SHOW Y_PREDS"""
                # dic_results['y_pred'] = dict_pred
                """ #####################################"""
                dic_results['totalTime'] = total_time
                dic_results['maxMem'] = getrusage(RUSAGE_SELF).ru_maxrss / (1024 * 1024)
                shutil.rmtree(dir_path) # REMOVING CACHE
                return dic_results
...
if __name__ == '__main__':
    ...
    # print(args)
    print(
        wise_SHsaint(args.device, args.num_layers, args.hidden_channels, args.dropout, args.lr, args.epochs, args.runs,
...
def inference_API(dataset_name,targetNodesList,model_id,):
    global args,SAMPLED_INFERENCE,RW_SAMPLER
    SAMPLED_INFERENCE = False
    RW_SAMPLER = False
    args = get_args(dataset_name)
    args.targetNodesList = targetNodesList
    if not model_id == '':
        args.model_id = model_id
    result = wise_SHsaint(args.device, args.num_layers, args.hidden_channels, args.dropout, args.lr, args.epochs, args.runs,
                 args.batch_size, args.walk_length, args.num_steps, args.loadTrainedModel, args.dataset_name,
                 args.root_path, args.output_path, args.include_reverse_edge, args.n_classes, args.emb_size,
                 modelID=args.modelID)
    return result
