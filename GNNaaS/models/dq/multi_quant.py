...
def evaluate_prob_mask(x_dict, prob_high=0.7):
    """
    Generates a binary mask based on a Bernoulli distribution.
    Nodes will be assigned higher precision with probability `prob_high`.
    """
    mask_dict = {}
    for key, x in x_dict.items():
        # Probability mask based on the number of nodes for each type
        mask = torch.bernoulli(torch.full((x.size(0),), prob_high)).to(torch.bool)
        mask_dict[key] = mask
    return mask_dict
