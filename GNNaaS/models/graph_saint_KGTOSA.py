...
def graphSaint(device=0, num_layers=2, hidden_channels=64, dropout=0.5,
    ...
    for GNN_dataset_name in GNN_datasets:
        ...
        if loadTrainedModel == 1:
            ...
            # dic_results["data_obj"] = data.to_dict()
            with open(os.path.join(KGNET_Config.trained_model_path, model_name) + ".param", 'wb') as f:
                pickle.dump(dict_model_param, f)
            with open(os.path.join(KGNET_Config.trained_model_path, model_name) + "_wise.param", 'wb') as f:
                pickle.dump(dict_model_param, f)
            del train_loader
    return dic_results
