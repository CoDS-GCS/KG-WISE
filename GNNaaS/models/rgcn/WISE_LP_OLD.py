...
def generate_inference_subgraph(master_ds_name, graph_uri='',targetNodesList = [],labelNode = None,targetNodeType=None,
    ...
    # print('here')
    if not master_ent_df.iloc[10,:].map(lambda x: '/' in str(x)).values.any() and inf_ent_df.iloc[-1,:].map(lambda x: '/' in str(x)).values.any():
        if master_ent_df.iloc[:,:].map(lambda x: '/' in str(x)).values.any():
            print(f'\n\nWARNING SOME OF THE ENTITIES IN MASTER CONTAIN "/" WHILE OTHERS DO NOT,\nThis may lead to incorrect mapping!\n\n')
        inf_ent_df = inf_ent_df.map(lambda x:str(x).split('/')[-1])

    if not master_rel_df.iloc[10, :].map(lambda x: '/' in str(x)).values.any() and inf_rel_df.iloc[-1:,:].map(lambda x: '/' in str(x)).values.any():
        if master_rel_df.iloc[:, :].map(lambda x: '/' in str(x)).values.any():
            print(
                f'\n\nWARNING SOME OF THE RELATIONS IN MASTER CONTAIN "/" WHILE OTHERS DO NOT,\nThis may lead to incorrect mapping!\n\n')
        inf_rel_df = inf_rel_df.map(lambda x: str(x).split('/')[-1])
...
def rgcn_lp(dataset_name,
    ...
    if loadTrainedModel == 1:
        ...
        # dict_model_param['hidden_channels'] = 100
        inference = 0
        if '_wise' in modelID:
            inference = 1
        model = GAE(
        ...
        if target_rel not in relations_dict:
            if target_rel.split('/')[-1] in relations_dict:
...
if __name__ == '__main__':
    ...
    # FOR WIKI-KG
    # master_dataset_name = r'mid-ddc400fac86bd520148e574f86556ecd19a9fb9ce8c18ce3ce48d274ebab3965'
    ...
    # list_src_nodes = pd.read_csv(os.path.join(root_path,master_dataset_name,'test.txt'),header=None,sep='\t')[0].tolist()[:400]
    ...
    # FOR YAGO-310
    master_dataset_name = r'KGTOSA_YAGO3-10'
    target_rel = r'http://www.yago3-10/isConnectedTo'
    modelID = r'KGTOSA_YAGO3-10.model' #
    graph_uri = 'http://www.yago3-10/'
    list_src_nodes = pd.read_csv(os.path.join(root_path,master_dataset_name,'test_QUERY.txt'),header=None,sep='\t')[0].tolist()[:400]

    # FOR DBLP
    # master_dataset_name = r'dblp_LP'
    ...
    for i in range(total_runs):
        ...
        if WISE:
            modelID = modelID.replace('.model', '_wise.model') if not modelID.endswith('_wise.model') else modelID
            dataset_name = generate_inference_subgraph(master_ds_name=master_dataset_name,
                                                       graph_uri=graph_uri,
        ...
        for i in range(total_runs):
            ...
            result, hits = rgcn_lp(dataset_name, root_path=root_path, target_rel=target_rel, loadTrainedModel=1,
                                   list_src_nodes=list_src_nodes, modelID=modelID, hidden_channels=128, K=K)
...
# Note
# For DBLP and wikidata , uncomment dict_model_param['hidden_channels'] = 100
# For YAGO, comment dict_model_param['hidden_channels'] = 100