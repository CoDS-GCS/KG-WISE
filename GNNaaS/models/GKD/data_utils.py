...
@torch.no_grad()
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None, test_dataset = None,y_true = None,subject_node = None,return_pred= False):
    #dataset  = [x_dict,edge_index_dict,key2int]
    ...
    if test_dataset == None: 
        ...
        test_out = model.inference([dataset[0],test_dataset,dataset[-1]], mode=args.mode) # teacher trained/valid on full dataset, test on mask dataset

    key2int = dataset[-1]

    train_acc = eval_func(y_true[split_idx['train'][subject_node]],out[str(key2int[subject_node])][split_idx['train'][subject_node]])

    valid_acc = eval_func(y_true[split_idx['valid'][subject_node]],out[str(key2int[subject_node])][split_idx['valid'][subject_node]])

    test_acc = eval_func(y_true[split_idx['test'][subject_node]],test_out[str(key2int[subject_node])][split_idx['test'][subject_node]])
    ...
    if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins'):
        ...
        if not return_pred:
            out[key2int[subject_node]] = F.log_softmax(out[str(key2int[subject_node])], dim=1)
            valid_loss = criterion(
                out[str(key2int[subject_node])][split_idx['valid'][subject_node]], y_true.squeeze(1)[split_idx['valid'][subject_node]])
        else:
            valid_loss = 0
    # if not return_pred:
